...
# This should repeat reliably.
# The problem:
# The bot properly greets and I hear it at start, but after replying, the bot does not start speaking again.
# We see how the stt works as the transcription is visible and correct.
# We see how the llm is working as it generates a response.
# But finally it doesnt speak.
# We dont see how that response is passed to the tts and spoken.
        ...
        try:
            if isinstance(frame, StartFrame):
                try:
                    ...
                    pass
            # Log TTS lifecycle frames to verify synthesis activity
            if isinstance(frame, TTSStartedFrame):
                try:
                    print("[PipecatDebug] OutputProcessor: TTSStartedFrame received")
                except Exception:
                    pass
            if isinstance(frame, TTSStoppedFrame):
                try:
                    print("[PipecatDebug] OutputProcessor: TTSStoppedFrame received")
                except Exception:
                    pass
...
class VoiceBridgeServer:
    def __init__(self, engine: Any, sample_rate: int, frame_ms: int, pace: bool, monitor_input: bool, greet_enabled: bool, bot_beep_on_connect: bool = False, log_latency: bool = False) -> None:
        self.engine = engine
        self.sample_rate = sample_rate
                ...
                try:
                    ...
                    while not debug_stop_event.is_set():
                        ...
                        if ch == 'q' or ch == 'Q':
                            ...
                            break
                        if ch == 't' or ch == 'T':
                            print("[Bridge:Debug] Bypass pipeline: PipecatPipelineEngine direct TTS")
                            def _do_direct() -> Any:
                                async def _coro2() -> None:
                                    try:
                                        try:
                                            # Generate test tone for 300ms @ 440Hz
                                            duration_s = 0.3
                                            sr = self.sample_rate
                                            num_samples = int(sr * duration_s)
                                            samples = bytearray()
                                            ...
                                            for n in range(num_samples):
                                                ...
                                                val = int(amplitude * math.sin(2 * math.pi * 440.0 * t))
                                                samples += int(val).to_bytes(2, byteorder='little', signed=True)
                                            await self._send_pcm(ws, bytes(samples), sr)
                                        except Exception as e2:
                                            print(f"[Bridge:Debug] Direct PipecatPipelineEngine error: {e2}")
                                    except Exception as e:
                        ...
                        if ch == 'l' or ch == 'L':
                            ...
                            if isinstance(self.engine, PipecatPipelineEngine) and self.engine._task is not None:
                                try:
        ...
        try:
            async for message in ws:
                ...
                if chunk:
                    ...
                    if isinstance(self.engine, PipecatPipelineEngine):
                        try:
...
def main(argv: Optional[list[str]] = None) -> int:
    try:
        ...
        engine = PipecatPipelineEngine(args.sample_rate)
        ...
        print(f"[Bridge] Starting on ws://{args.host}:{args.port}/attendee-websocket engine=pipecat sr={args.sample_rate} frame_ms={args.frame_ms} pace={not args.no_pace} greet={True} monitor={args.monitor_input} bot_beep_on_connect={args.bot_beep_on_connect} log_latency={args.log_latency}")
